...
def parse_one_page(text):
	...
	#获取电影排名信息
	indexs = soup.find_all(name = 'i',attrs = {'class':'board-index'});
	indexs = get_values_from_tag(indexs,'content');
	img_srcs = soup.find_all(name = 'img',attrs = {'class':'board-img'});
	img_srcs = get_values_from_tag(img_srcs,'data-src');	
	names = soup.find_all(name = 'p',attrs = {'class':'name'});
	names = get_values_from_tag(names,'content');
	actors = soup.find_all('p',attrs = {'class':'star'});
	actors = get_values_from_tag(actors,'content');
	times = soup.find_all('p',attrs = {'class':'releasetime'});
	times = get_values_from_tag(times,'content');
	integers = soup.find_all('i',attrs = {'class':'integer'});
	integers = get_values_from_tag(integers,'content');
	fractions = soup.find_all('i',attrs = {'class':'fraction'});
	fractions = get_values_from_tag(fractions,'content');
	for index in range(len(indexs)):
		yield {
			...
			'score':integers[index].strip() + fractions[index].strip()
		}
